...
class UNETR(nn.Module):
    def __init__(self, img_shape=(128, 128, 128), input_dim=1, output_dim=3, embed_dim=768, patch_size=16, num_heads=12, dropout=0.1):
        super().__init__()
        ...
        self.pre_process_cnn = \
            nn.Sequential(
                nn.Conv3d(in_channels=1, out_channels=3, kernel_size=3, stride=1, padding='same'),
            )

    def forward(self, x):
        pre_z = self.pre_process_cnn(x)

        z = self.transformer(pre_z)
        z0, z3, z6, z9, z12 = pre_z, *z
