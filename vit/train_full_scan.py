...
img_paths = [os.path.join(dataPath, 'processed_scan', f) for f in os.listdir(os.path.join(dataPath, 'processed_scan'))]
img_paths = [p for p in img_paths if not p.endswith('.json')]

label_path = os.path.join(dataPath, 'csv', 'annotations.csv')

dataset = ScanDataset(img_paths=img_paths, label_path=label_path)
