...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=96, patch_size=8, in_chans=1, embed_dim=1024, norm_layer=None):
        super().__init__()
...
def window_partition(x, window_size):
    ...
    windows = x.permute(0, 1, 3, 2, 4, 5, 6).contiguous().view(-1, window_size, window_size, window_size, C)
    return windows
...
def window_reverse(windows, window_size, H, W, D):
    ...
    x = x.permute(0, 1, 3, 2, 4, 5, 6).contiguous().view(B, H, W, D, -1)
    return x
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # cyclic shift
        if self.shift_size > 0:
            shifted_x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2, 3))
        else:
            shifted_x = x
